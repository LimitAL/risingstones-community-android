...
状态：动态互动、动态发布、帖子/攻略转发及五类招募转发已补齐原生流程；帖子与攻略自身的
发布编辑、幻化同步、动态再次转发及真实账号写入仍待补或待验证。

- [ ] 发布、编辑、删除帖子与攻略；富文本、图片及其他官方支持内容的编辑与上传。
- [ ] 发起投票并配置选项与规则，区别于已有的参与投票服务。
- [x] 发布动态，转发已确认的帖子与攻略来源；支持三档可见范围、提及、官方表情及最多九张有序图片。
- [x] 五类招募使用独立确认页转发；精确发送 `recruit/relay` 的 `from/scope/recruid_id`，不复用帖子字段。
- [ ] 幻化投稿或编辑时的公开同步归入投稿流程；动态再次转发仍缺当前入口和写契约证据。
- [x] 动态正文点赞、评论、楼中楼回复、删除本人动态与评论；支持单图、官方表情和关注对象提及。
- [x] 动态发布编辑中退出确认、提交失败保留草稿、明确重试及成功后的宿主回流回调。
- [ ] 补齐帖子/攻略编辑流程和本人历史中的对应管理入口。
...
动态通过独立可选[操作服务](../dynamic-domain/src/main/java/top/cxmeow/risingstones/feature/dynamic/domain/DynamicActions.kt)
扩展既有关注流、详情、评论与回复读取，精确请求、能力及验证设计见[动态互动设计](dynamic-actions-design.md)。
官网动态详情明确关闭评论点赞，不能将共享组件的帖子评论点赞接口视作动态评论接口。
动态发布与帖子/攻略转发复用同一凭证作用域和 `DynamicWrite`，精确表单、图片用途隔离及原生
编辑流程见[动态发布与帖子转发设计](dynamic-publishing-design.md)。帖子/攻略转发只接受已读取的
正整数帖子 ID。五类招募转发通过可选服务复用 `DynamicWrite`，确认页只有可见范围，没有正文、
提及或图片；`recruid_id` 保留官网拼写，成功不假定返回新动态 ID。部队来源仍要求当前招募读取
身份有效，详见[招募分享到动态设计](recruitment-relay-design.md)。幻化同步和动态再次转发继续
保留现有证据边界。本批未用真实账号执行发布、上传、改变可见范围或转发，本批设备、全量构建
和公共接入验证见[兼容性矩阵](compatibility-matrix.md)。
此前动态互动批次已覆盖取消、失败重试、发送中配置重建、原生表情、图片方向与四个宽度断点；
换号与能力撤销会清除会话专属状态、旧作用域和迟到图片。本批发布与转发的设备和全量验证
统一在本轮完成后汇总，不提前写入通过数量。
...
- [x] 核对 `/search`、`/searchm`、`/sresult` 的完整搜索分类与当前匿名响应边界；论坛标题／
  正文搜索补齐原生选择，修正攻略标题误用帖子正文类型的问题。
- [ ] 全站用户搜索及统一搜索入口仍待接入；幻化标题和物品候选搜索已有领域入口，不等同于
  已完成统一搜索。官网搜索可选 type 为 1、2、3、4、6、7、8、9、10，type 5 不伪造入口。
  装备和配饰候选接口匿名返回 10403，不能因搜索路由无需登录就开放这些鉴权查询。
