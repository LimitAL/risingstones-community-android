...
import top.cxmeow.risingstones.feature.forum.presentation.OfficialForumCommentTextChange

internal val LocalForumCommentEmojiLoader = staticCompositionLocalOf<suspend (Int) -> Bitmap?> {
    { number -> RisingStonesForumImageLoader.load(OfficialForumResourceUrls.emojiImageUrl(number))?.asAndroidBitmap() }
}

internal val ForumCommentEmojiPattern = Regex("\\[emo([1-9]|[1-3][0-9]|4[0-6])]")

private fun OfficialForumCommentEditorState.emojiMatches(): List<MatchResult> {
    val protectedNames = mentions.filter { it.start >= 0 && it.end <= text.length && it.start < it.end &&
        text.substring(it.start, it.end) == "@${it.mention.name}" }
    return ForumCommentEmojiPattern.findAll(text).filter { match -> protectedNames.none {
...
@Composable
internal fun ForumCommentTextEditor(
    state: OfficialForumCommentEditorState,
    enabled: Boolean,
    onChange: (String, Int, Int, OfficialForumCommentTextChange?) -> Unit,
) {
    ...
    var field by remember { mutableStateOf<ForumNativeCommentEditText?>(null) }
    ...
    val hint = stringResource(R.string.forum_comment_hint)
    val emojiLabels = (1..46).associateWith { stringResource(R.string.forum_comment_emoji_number, it) }
    Surface(shape = MaterialTheme.shapes.extraSmall,
        ...
        AndroidView(
            modifier = Modifier.fillMaxWidth().heightIn(min = 112.dp, max = 240.dp)
                .testTag("forum-comment-input").semantics {
            ...
            factory = { context -> ForumNativeCommentEditText(context).also { editor ->
                field = editor
...
internal class ForumNativeCommentEditText(context: Context) : EditText(context) {
    var onEditorChange: ((String, Int, Int, OfficialForumCommentTextChange?) -> Unit)? = null
    private var applying = false
    private var textChanging = false
    private var pendingChange: OfficialForumCommentTextChange? = null
    ...
    init {
        tag = "forum-comment-native-input"
        background = null
        ...
        addTextChangedListener(object : TextWatcher {
            override fun beforeTextChanged(s: CharSequence?, start: Int, count: Int, after: Int) {
                if (!applying) { textChanging = true; pendingChange = OfficialForumCommentTextChange(start, start + count) }
            }
    ...
    private fun emitChange(change: OfficialForumCommentTextChange?) {
    ...
    fun render(state: OfficialForumCommentEditorState, images: Map<Int, Bitmap>, emojiSize: Int,
        ...
        try {
            ...
            state.emojiMatches().forEach { match ->
                ...
                val drawable = images[number]?.let { BitmapDrawable(resources, it) }
                    ?: ForumEmojiPlaceholder(number, placeholderColor, foreground)
...
private class ForumEmojiPlaceholder(private val number: Int, private val background: Int, private val foreground: Int) : Drawable() {
